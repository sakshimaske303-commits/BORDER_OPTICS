...
Finally, the built-up-area result can be summarily said to be compositing-window sensitive and not confirmed. There is no significant change seen in a full-year composite, but there is a very significant change in the same villages recomposited to a June–September summer-matched window. The dichotomy traces to the type of artifact present on either side: snow-cover contamination in the full-year window, and monsoon cloud cover in the summer window, effectively eliminating Sikkim's data. Three additional checks were applied to this significant summer result, and it survived all three — it is significantly larger than the change seen in a district-restricted, district-FE non-VVP control group (p = 0.00033), it holds at 250m and 1km buffer radii as well as 500m once the comparison is restricted to the 154 villages with valid data at all three radii (the raw, unmatched 250m/1km extractions are not significant on their own — see the paper's Section 4.8 for both sets of numbers), and it survives Holm-Bonferroni correction for multiple testing (raw p = 9.29×10⁻¹⁵, Holm-adjusted p = 7.43×10⁻¹⁴). The control-group check was itself pushed further still: dropping any single one of the 14 districts and rerunning leaves it significant every time, and a randomization-inference test that makes no assumption about the number of clusters agrees (p = 0.0005) — see the paper's Section 4.6. It isn't a steady trend, though: a three-point 2021/2023/2025 extension reveals the bulk of the change is concentrated in a 2023-to-2025 recovery. Night-lights' own before/after change stays a null throughout (below), but its control-group DiD tells a different story — see the row below and Section 4.6 of the paper for why that isn't a contradiction.
...
| Control-group DiD (NDBI, H4) | p = 0.275 — not significant | p = 0.00033 — significant |
| Control-group DiD (Night-lights, H4) | p = 0.036 (FE) / p = 0.103 (no-FE) — spec-dependent | p = 0.0044 — significant (both specs) |
...
* ✓ District-restricted non-VVP control group (735 villages, same 14 districts, DiD design)
* ✓ Control-group DiD checked further: leave-one-district-out (significant in all 14/14) and randomization inference (p = 0.0005)
...
This is not a clean confirmation, this is an instability disclosure. The Himalayas introduce their own seasonally driven measurement artifacts: snow cover in winter months confounds the full-year window, and monsoon cloud cover wipes out usable summer data for Sikkim entirely. Those artifacts alone are enough to make a single compositing choice unreliable evidence. Comparing to the control villages strengthens the remaining signal, but the comparison itself rests on a baseline imbalance relative to the control group in three of the four outcome/window pairs tested (NDBI summer, and both night-lights windows — night-lights full-year most severely, which is also the one combination where the two DiD specifications disagree on significance), without any actual proof of a common pre-trend. Separately, the 154 villages that ended up with valid summer-window data are not a random 154 out of the 251-village core sample — they differ significantly from the 97 villages that dropped out, both in which state they're from (mostly Sikkim's total dropout) and in their own 2021 baseline NDBI (p = 0.00063) — so the summer-window result describes this specific subsample, not the full core sample by default (see the paper's Section 6.4). One open question about that same subsample, found but not yet resolved: Sikkim's "zero valid summer data" looks, on comparison with a separately-run extraction for a different section of the paper, like it may be a snapshot of the satellite archive on the day this was extracted rather than a permanent fact — see the paper's Section 6.4 and Development Log Entry 21 for what was found and what re-checking it would take. Due to limitations on sample size and geometry, the "border-proximity" correlation (H3) is treated as exploratory rather than confirmatory. The data gaps – the "unresolved" villages in Ladakh, the "partial" sample in HP, and the "block" confusion in Uttarakhand – are not glossed over here.
