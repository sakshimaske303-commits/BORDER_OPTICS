...
**Not done in this entry.** The larger Anara-suggested extensions (covariate matching, Cloud Score+, DW thresholds, extended panel, visual audit) remain logged as future work, not started. The RTI remains unfiled — now disclosed as a permanent scope boundary rather than an open item, per the decision above. (Correction: this entry originally said the PDF rebuild was "deferred" — it was not; `build_docs_pdfs.sh` was run and all three PDFs, root and `static/`, were rebuilt and synced within this same entry, before Sakshi asked in the next round to stop doing that and handle PDF export herself from the `.md` files going forward. See Entry 35.)

## Entry 35

**Status.** Sakshi asked for the three dark-themed dashboard-style figures (04/05/06) to be remade in the same light, professional style as the paper's other figures, for uniform diagrams — plus a general re-check of README/CITATION/paper/dashboard. Separately, she ran a third round of external review (Anara again) against her own committed git state, and pasted its findings.
...
1. **Confirmed, real, and this project's own mistake.** `outputs/scl_vs_qa60_comparison.json` as delivered to the device after the Entry 34 script fix still had the *old* hardcoded `coverage_note` narrative text (the one describing SCL as significant, p=0.0000045, "mask choice... drives the difference") even though its numeric fields were already correct. Root cause, confirmed by checking this session's own history: the fixed `scl_vs_qa60_comparison.py` was rerun and verified in the sandbox after the Entry 34 fix, producing the correct JSON (`masks_agree_on_significance`, `coverage_diff`, `qa60_wilcoxon_p_full` all present, stale narrative removed) — but that regenerated JSON was never actually sent to the device in Entry 34's sync batch, only the fixed *script* was. The file on disk was therefore still the one Sakshi's own terminal run had produced from the *old* version of the script, before the narrative-text fix. Caught by an external reviewer reading the committed file, not by this project's own process. Fixed by re-syncing the correct, already-regenerated JSON (no re-extraction needed — the underlying numbers were never wrong, only the delivery step was incomplete).

2. **Not yet independently confirmed.** Anara also reported that commit `2217be9` deletes `static/BO_Research_Paper.pdf`, `static/BO_Executive_Summary.pdf`, and `static/BO_Development_Log.pdf` while `app.py`/`utils/doc_viewer.py` still reference them, implying dead links on the live dashboard. Checked what this session actually delivered: all three `static/` PDFs were rebuilt via `build_docs_pdfs.sh` and synced to the device with zero rejections in Entry 34, timestamped after the SCL-fix content changes, and `app.py`'s three filenames match exactly what's in `static/`. No `.gitignore` rule excludes PDFs or `static/`. So if those files are genuinely missing from Sakshi's committed git history, it happened in her own `git add`/`commit` step on her machine, not in what this session wrote to disk — this needs Sakshi to check (`git show --stat 2217be9 -- static/` or equivalent) before treating it as confirmed, rather than assumed from Anara's text alone.
...
**Not done in this entry.** The larger Anara extensions from Entry 34 (covariate matching, Cloud Score+, DW thresholds, extended panel, visual audit) remain future work, not started. The `static/` PDF deletion claim is unresolved pending Sakshi's own git check. No PDFs were rebuilt in this entry — per Sakshi's instruction, PDF export is now her own step from the `.md` files; this session edits `.md`/data/code only.

## Entry 36

**Status.** A fourth round of external review (Anara again) proposed eight suggestions to strengthen the paper academically: a district-verification rerun, a Dynamic World 2019 placebo test, equivalence framing for the null, a geocoding selection-bias table, a SUTVA sentence, a multiple-testing family statement, a results-stability table, and a pre-registration substitute. Sakshi asked for a genuineness assessment of all eight, then selected four to implement directly in this sandbox (the two requiring live network/GEE access were correctly identified as out of reach here and deferred to her own machine — see Entry 37): a SUTVA/spillover sentence, a multiple-testing family clarification, a pre-registration-substitute framing, and a geocoding selection-bias check restricted to District/Block/Population (not the altitude/border-distance comparison Anara originally suggested, since ungeocoded villages have no coordinates to compute either from). This entry was carried out but never logged at the time — recorded here after the fact, dated to when the work was actually done.
...
**A course correction mid-round, worth recording.** After delivering these four additions plus, separately, three standalone draft files (`BO_Results_Stability_Table.md`, `BO_Equivalence_Framing_Draft.md`, `BO_Pitch_Document.md`) covering the remaining Anara suggestions not selected for direct implementation, Sakshi pushed back specifically on the equivalence-framing draft: she had not asked for it to be created. Corrected non-defensively, offered to delete the unwanted drafts, and for the rest of this round's work, edited directly into `BO_Research_Paper.md` itself rather than generating further standalone draft files — the four additions above followed that corrected approach.
...
**Not done in this entry.** The two suggestions requiring live network/GEE access — a district-verification rerun and a 2019 Dynamic World placebo test — were correctly identified as infeasible in this sandbox and deferred; see Entry 37 for the district-verification rerun, carried out on Sakshi's own machine. The Dynamic World 2019 placebo test's extraction scripts were written and delivered in the same window as Entry 37 but had not yet been run as of that entry.

## Entry 37

**Status.** Sakshi ran the district-verification rerun deferred from Entry 36, on her own machine with live Overpass/Nominatim access. This entry covers that rerun, a real resume-logic bug it surfaced across three extraction scripts (and, caught while checking for the same pattern, in two more scripts that hadn't been run yet), the full control-group re-extraction the fix required, and every downstream paper number that changed as a result.
...
**Not done in this entry.** The Dynamic World 2019 placebo test (Entry 36's fourth deferred item) has its extraction and analysis scripts written and fixed, but the 2019 baseline extraction itself has not been run — needs `extract_dw_pretreatment_baseline.py` on Sakshi's own machine for both groups/windows, then `dw_pretreatment_placebo_test.py`. `extract_pretreatment_baseline.py --group control` and `extract_building_footprints.py --group control` also still need re-running against the current 721-village list before `pretreatment_placebo_test.py` (existing §7.5 result) or the control-side building footprints (currently unused) can be trusted if re-run. Sikkim's North district remains permanently or provisionally bbox-only; no further attempt was made in this entry to resolve it differently (e.g. by aggregating known constituent village names).
