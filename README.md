...
Against a district-restricted non-VVP control group of 735 villages across the same region, NDBI results from the summer-matched sample are statistically different (district-fixed-effects DiD, p = 0.00033), whereas those from the full-year sample are not (p = 0.275) — the same window-sensitivity pattern, now shown to hold after netting out the district-level baseline differences and the common regional trend that both treated and control villages shared over the same period (district fixed effects; not a district-specific trend estimate — see the Research Paper's Section 3.7 for the distinction). This also holds at 250m and 1km buffer radii, so it isn't an artifact of the 500m radius used everywhere (all p < 0.002 on a sample-matched comparison), and it survives Holm-Bonferroni correction for multiple testing. The control-group DiD result was pushed further still: it stays significant when any single one of the 14 districts is dropped and rerun (14/14), and a randomization-inference test that doesn't assume many clusters agrees (p = 0.0005) — see the Research Paper's Section 4.6. Night-lights, which shows no absolute before/after change of its own in either window, does show a significant positive gap against this same control group in both windows (summer p = 0.0044, robust to specification; full-year p = 0.036 under fixed effects but not under the no-fixed-effects comparison) — driven by control villages declining rather than treated villages rising; see the Research Paper's Sections 4.6/6.7 for the baseline-imbalance caveat that applies most strongly to the full-year result.
