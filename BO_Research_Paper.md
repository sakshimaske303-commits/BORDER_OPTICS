...
The extent of built-up change was not correlated with distance to the border/LAC within the sample in either window - full-year ρ = 0.049 (p = 0.443) and summer-matched ρ = 0.038 (p = 0.641), which remained a null in each case regardless of which window was selected. The results for night-lights were more complex—a strong relationship in the full-year window towards more change in the closer villages (ρ = -0.252, p < 0.0001, H3 prediction) with no such relationship in the summer matched window (ρ = -0.071, p = 0.266). As described in Section 4.2, this window-sensitivity has been noted by some, but is not confirmed by any.
...
### 4.6 Control-Group Difference-in-Differences: Two Gaps That Hold, Against Baselines That Aren't Balanced (H4)
...
Against the matched 735-village control group, the summer-window NDBI gap still holds up, and holds up somewhat more tightly than before: did coefficient = +0.0322, 95% CI [+0.0146, +0.0498], cluster-robust p = 0.00033 (HC3 no-fixed-effects comparison: +0.0271, p = 0.0124). This regression draws on 220 treated villages — any village with a valid NDBI reading in at least one of the two periods, since the panel isn't balanced — against 250 control villages with a valid summer-window reading (out of 735; the same cloud/snow dropout that thins the treated sample to 154 villages for the stricter baseline-balance check below applies equally to the control group, and thins it further). As before in Section 4.2, the full-year gap is not significant (+0.0088, p = 0.275, n=190 control villages with valid full-year data) — this is the design which also accounts for the regional trend.

Night-lights is where the corrected control group changes the picture, not just the precision of it. Under the OLD, bugged 753-village list, no DiD effect was observed in night-lights in either window. Under the corrected 735-village list, a gap now shows up in both: summer +0.1413, 95% CI [+0.0442, +0.2385], cluster-robust p = 0.0044 (HC3: same coefficient, p = 0.0019 — both specifications agree here); full-year +0.1653, 95% CI [+0.0105, +0.3202], cluster-robust p = 0.0364, but the HC3 no-fixed-effects comparison does not agree (p = 0.1033). This is not a case of treated villages' own night-lights actually rising — Section 4.3's treated-only Wilcoxon test on the exact same summer window is a clean null (p = 0.9999, essentially flat) — it is a case of the *control* villages' night-lights falling over the same period while treated villages stayed roughly flat, so the treated-minus-control gap comes out positive without treated villages showing any absolute increase of their own. Read this as "VVP-I priority villages held steady while the surrounding non-priority region's night-lights declined," not as "VVP-I priority villages lit up."

There is a caveat, however, underlying both outcomes, and it's more severe for night-lights than it was reported to be for NDBI before this correction. Treatment and control villages show significant differences at baseline (2021) in three of the four outcome/window combinations: NDBI summer (treated mean −0.2461 vs. control mean −0.2442, n=154/250, Mann-Whitney p = 0.00073 — a much smaller mean gap than the earlier, bugged run reported, though still statistically significant), night-lights full-year (treated mean 0.4282 vs. control mean 0.4082, n=251/190, p < 0.00001 — the most imbalanced of the four, and the one combination where the two DiD specifications disagree on significance, which is at least consistent with a baseline-imbalance explanation for that disagreement), and night-lights summer (treated mean 0.4017 vs. control mean 0.3656, n=251/250, p = 0.0165). Only NDBI full-year is not significantly imbalanced at baseline (treated −0.1846 vs. control −0.2214, n=251/190, p = 0.125). None of this is surprising on its own terms — as noted above, VVP-I priority villages were chosen in part because they were in some way more remote — but it means every DiD estimate in this section rests on a level difference rather than a directly confirmed shared pre-trend, since a genuine multi-period pre-treatment panel wasn't extracted for the control group (see Section 3.7), and the night-lights full-year result specifically should be read as spec-dependent rather than as a settled finding. Per-district, the NDBI summer gap is positive in 2 of the 3 districts with enough summer-window coverage on both sides to report a per-district figure, and the night-lights summer gap is positive in 4 of the 5 such districts — smaller district counts than the corrected control group's overall size would suggest, since the same coverage-threshold filtering that shapes the headline n's also thins out which individual districts have enough valid villages on both sides to break out separately.
...
The H4 comparison in Section 4.6 revealed that the extracted mean NDBI and night-lights values for treated and control villages in 2021 are significantly different from each other in three of the four outcome/window combinations tested (NDBI summer, night-lights full-year, and night-lights summer; NDBI full-year is the one combination without a significant baseline gap), and the parallel comparison could not be conducted by performing a placebo test (no pre-treatment village). Night-lights full-year carries the largest imbalance of the four (p < 0.00001) and is also the one combination where the district-fixed-effects and no-fixed-effects specifications disagree on significance — a pattern consistent with, though not proof of, the fixed-effects specification's district-level adjustment partially absorbing a baseline gap that the no-fixed-effects specification cannot. In this approach district fixed effects control for district-level differences in terms of district-level baseline selection into the treated group and do not adjust the selection of the group at the village level. Interpret the DiD estimate as provided by a control-group adjusted natural experiment, not as a fully clean natural experiment — and interpret the full-year night-lights result specifically as the more fragile of the two night-lights findings, given both this baseline gap and its spec-dependent significance.
