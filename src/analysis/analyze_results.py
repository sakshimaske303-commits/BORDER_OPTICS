...
df = pd.read_csv("data/processed/border_optics_village_results_summer.csv")
...
print(f"NDBI test run on {len(valid_ndbi)}/{len(core)} villages with valid data "
      f"(rest excluded — no cloud-free imagery in the summer window)")
...
print("EXPLORATORY ONLY — n=3 states, not a statistically powered test")
print("=" * 60)
...
df.to_csv("data/processed/border_optics_village_results_summer_analyzed.csv", index=False)
print("\nSaved enriched dataset to data/processed/border_optics_village_results_summer_analyzed.csv")