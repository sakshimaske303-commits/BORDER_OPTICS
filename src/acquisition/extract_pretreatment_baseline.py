"""
2019 baseline for the placebo test (Section 7.5) -- gives me a second
pre-treatment point (2019, 2021) for both groups, before VVP-I even existed.
Treated-only multiyear script can't do this since control has no 3rd point.

Needs live GEE, run on my machine.

python3 src/acquisition/extract_pretreatment_baseline.py --group treated --window full_year
python3 src/acquisition/extract_pretreatment_baseline.py --group treated --window summer
python3 src/acquisition/extract_pretreatment_baseline.py --group control --window full_year
python3 src/acquisition/extract_pretreatment_baseline.py --group control --window summer

Then run pretreatment_placebo_test.py once all 4 exist.
"""
import argparse
import os
import time
...
# single composite per village here, not before/after -- 2019 IS the "before" for this placebo
WINDOWS = {
...
def _load_with_resume(group, out_path):
    # treated village_id is stable so resume by id; control list gets regenerated
    # each time so resume by coords instead (same fix as extract_control_satellite_data.py)
